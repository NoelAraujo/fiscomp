...
(@) Vamos achar o máximo com conhecimentos de Cálculo 1: o máximo (ou minimo, ou inflexão) de uma função $f(x)$ existe quando a primeira derivada é nula, $f'(x^*) = 0$.
    ...
    a. Diminua a magnitude dos dados com uma normalização do tipo $f \rightarrow f/max(f)$

    a. Crie um vetor com os valores analíticos e também os normalize

    a. Compare com os valores numéricos com os analíticos em uma figura
